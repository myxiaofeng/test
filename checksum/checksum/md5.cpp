 ...
 * of bytes. 
 */  
void MD5::MD5Update(MD5_CTX *ctx, unsigned char const *buf, unsigned len) {  
    uint32 t;  
 ...
 */  
void MD5::MD5Final(unsigned char digest[16], MD5_CTX *ctx) {  
    unsigned count;  
    unsigned char *p;  
 ...
 */  
void MD5::MD5Transform(uint32 buf[4], uint32 const in[16]) {  
 ...
 */  
int MD5::getBytesMD5(const unsigned char* src, unsigned int length, char* md5){ 
    unsigned char i = 0;  
 ...
 */  
int MD5::getStringMD5(const char* src, char* md5) {  
 ...
 * get MD5 of a file 
 */  
int MD5::getFileMD5(const char* filename, char* md5) {  
    string path;
    FILE* fp = NULL;  
    ...
    if (fp == NULL) {  
        cout << __FUNCTION__ << " " << __LINE__ << " " << path << endl;
        return -1;  
    }  
    MD5Init(&context);  
...
int MD5::getCatlogMD5Cur(MD5_CTX *ctx, const char* path)
{
    FILE* fp = NULL;  
    unsigned char buffer[MD5_FILE_BUFFER_LEN] = { 0 };  
    int count = 0;   
    ...
    {
        dealNullFileCat(buffer, finfo);
        MD5Update(ctx, buffer, SECTIONSIZE); 
    }
    for(unsigned int i = 0; i < vfinfo.size(); i++)
    {
        ...
        {
            getCatlogMD5Cur(ctx, finfo.filename.c_str());
        }
        else
        {
            if(finfo.size == 0)
            ...
            while ((count = fread(buffer, 1, MD5_FILE_BUFFER_LEN, fp)) > 0) {  
                ...
                memset(buffer, 0, MD5_FILE_BUFFER_LEN);
            }    
        }     
    }
    return 0;
...
int MD5::getCatlogMD5(const char* filename, char* md5)
{
    string path;
    MD5_CTX ctx;
    if (filename == NULL || md5 == NULL) {  
    ...
    unsigned char md5Bytes[16] = { 0 };  
    MD5Init(&ctx);  
    if(getCatlogMD5Cur(&ctx, path.c_str()) < 0)
        return -1;
    MD5Final(md5Bytes, &ctx);  
    for (int i = 0; i < 16; i++) {  
        sprintf(md5, "%02X", md5Bytes[i]);  
        md5 += 2;  
    }  
